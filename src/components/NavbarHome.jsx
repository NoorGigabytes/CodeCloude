...
function NavbarHome() {
  ...
  return (
    <>
    <div className={`fixed left-0 right-0 top-0 z-50 h-15 pt-4 md:pb-4 pb-5 transition-all duration-300 ${scroll ? 'bg-white drop-shadow-md' : 'bg-transparent pt-7'}`}>
      <nav className="relative flex items-center container mx-auto h-full justify-between">
        ...
        <div>
          <div onClick={showMenu} className="cursor-pointer lg:hidden">
            {menuShow ? <IoMdClose className={`text-3xl ${scroll ? 'text-black' : 'text-white'}`}/> : <RiMenu4Line className={`text-3xl ${scroll ? 'text-black' : 'text-white'}`}/>}
          </div>
          <ul className={`hidden lg:flex items-center lg:space-x-5 xl:space-x-10 xl:text-lg lg:bg-transparent bg-sky-400 lg:text-md font-semibold transition-all duration-400 ${scroll ? 'text-black' : 'text-white'}`}>
